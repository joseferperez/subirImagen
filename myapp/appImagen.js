...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'imagen/') // Asegúrate de que este directorio exista
  },
...
app.get('/api/documentos', async (req, res) => {
  try {    
    //const { rows } = await pool.query(' SELECT nombre_original,url,tamano, tipo FROM catalogo.documentos where tipo = image/jpeg'); // Asegúrate de que tu tabla se llame 'personas'
    const { rows } = await pool.query("SELECT nombre_original, url, tamano, tipo FROM catalogo.documentos WHERE tipo = 'image/jpeg' or tipo = 'image/png' ");
